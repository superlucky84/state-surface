...
const ActionsLog = ({ entries }: LogProps) => (
  <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <h3 class="mb-4 text-lg font-semibold text-slate-900">Action Log</h3>
    {entries.length === 0 ? (
      <p class="text-sm text-slate-500">No actions triggered yet. Try the buttons above.</p>
    ) : (
      <ul class="space-y-3">
        {entries.map(e => (
          <li key={e.id} class="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
            ...
            <div class="min-w-0">
              <p class="text-sm font-medium text-slate-700">Action: {e.action}</p>
