...
## Phase 2-13.5: 플러그인 사용 가이드/예제 강화

> 설계: `DESIGN_PHASE2.md` §2.6

**Entry**: Phase 2-13 완료.
**Exit**: 플러그인 사용 기준이 가이드에 문서화되고, 예제 페이지에서 사용법을 재현 가능.

### Guide 문서 강화

- [ ] `shared/content.ts`의 template guide(또는 동급) 섹션에 `Plugin Patterns` 블록 추가:
  - 플러그인 사용 기준 (DOM 보정 vs 상태 로직)
  - anti-pattern (`onUpdate`에서 상태 결정/네트워크 호출 금지)
  - `prismPlugin`, `guideTocPlugin` 최소 예시
- [ ] en/ko 가이드 구조 parity 유지 (`shared/guideContent.test.ts` 기준).

### Example 페이지 추가

- [ ] `/features/plugins` (또는 동급) 예제 페이지 추가:
  - Prism 하이라이팅 플러그인 동작
  - 모바일 TOC active auto-scroll 플러그인 동작
- [ ] 예제에서 플러그인 미적용 대비 동작 차이를 설명하는 UI/텍스트 제공.

### 템플릿/스캐폴딩 동기화

- [ ] `create-state-surface/template`에도 동일 가이드/예제 반영.
- [ ] `create-state-surface/templateParity.test.ts`에 신규 동기화 파일 반영.

### Baseline 테스트

- [ ] `pnpm test -- shared/guideContent.test.ts` 통과 (가이드 schema/i18n parity 유지).
- [ ] `pnpm test -- engine/server/phase213Smoke.test.ts` 통과 (Guide/플러그인 스모크 유지).
- [ ] `pnpm test -- create-state-surface/templateParity.test.ts` 통과 (기준-템플릿 동기화).
- [ ] `pnpm dev`에서 `/guide/template` 및 `/features/plugins` 수동 확인:
  - 플러그인 사용 기준 문구 노출
  - 모바일 TOC active scroll 동작
  - Prism 하이라이팅/라인넘버 정상.
...
- [ ] 플러그인 사용 가이드/예제 문서화 완료 (`guide/template` + `/features/plugins`).
...
| 15     | **2-13.5 플러그인 가이드/예제 강화**      | Medium           | §2.6        | 도입/온보딩 품질                           |
...
- **Done**: Phase 2-1, 2-1.5, 2-2, 2-3, 2-4, 2-5, 2-6, 2-7, 2-8, 2-9, 2-10, 2-11, 2-12, 2-13 완료.
- **Next**: Phase 2-13.5 (플러그인 가이드/예제 강화).
