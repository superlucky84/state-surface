...
- [x] Action: search/features/chat 페이지 `data-action` end-to-end.
...
- [x] 프로덕션 검증: 스캐폴딩 앱에서 `pnpm build && pnpm start` 후 `GET /search`, `POST /transition/search` 정상.
- [x] 서브패스 검증: `BASE_PATH=/demo node --import tsx server.ts` 후
  - `GET /demo/search` 200, `GET /search` 404
...
- [x] 요청 로그 포맷 검증: `engine/server/logger.test.ts`에서 `[HTTP] GET /search 200` 검증.
