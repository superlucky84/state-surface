...
- [x] `create-state-surface/template/`에 현재 프로젝트 구조 반영.
- [x] 불필요한 파일 제외 (`.git`, `node_modules`, `dist`, `IMPLEMENT*.md`, `DESIGN*.md`, `PROTOCOL.md` 등).
- [x] `package.json.template` — 프로젝트명 치환 + `state-surface` 의존성 버전 범위 삽입.
- [x] 템플릿에 `server.ts` (사용자 서버 엔트리), `client/main.ts` (사용자 클라이언트 엔트리) 포함.
- [x] 템플릿에 `routes/_shared/hooks.ts` (i18n 훅) 포함.
- [x] 템플릿에 `client/plugins/prism.ts` (Prism 플러그인) 포함.

### CLI 구현/배포 설정

- [x] `bin/create-state-surface.js` — 프로젝트명 입력 → 디렉토리 생성 → 파일 복사 → 치환.
- [x] 프로젝트명/설명 치환 처리.
- [x] Git 초기화 (`git init`) 자동 실행.
- [x] 완료 후 안내 메시지 출력.
- [x] `create-state-surface/package.json` — `"name"`, `"bin"`, `"files"`, `"publishConfig"`.
- [x] `.npmignore` 또는 `"files"` 화이트리스트.
- [x] `npm pack` → 로컬 설치 검증.
...
- [x] 템플릿-기준 사이트 시각 패리티 테스트 추가 (`create-state-surface/templateParity.test.ts`).
...
- [x] `create-state-surface/template/shared/content.ts` 동기화.
- [x] `create-state-surface/template/shared/guideContent.test.ts` 동기화.

### Baseline 테스트

- [x] `pnpm test -- shared/guideContent.test.ts` 통과 (가이드 schema/i18n parity 유지).
- [x] `pnpm test -- engine/server/phase213Smoke.test.ts` 통과.
- [x] `pnpm test -- create-state-surface/templateParity.test.ts` 통과 (기준-템플릿 동기화).
