...
- [x] **DC-04** E2E 테스트 도구: **생략(C)** 확정 (v0.1.0은 통합 테스트 + 수동 검증 유지, E2E는 후속 단계에서 재평가)
- [x] **DC-05** 업데이트 경로: **재스캐폴딩 금지, 패키지 업그레이드 우선** (`pnpm up state-surface`) 확정 → `DESIGN_PHASE2.md` §6.4
- [x] **DC-06** 플러그인 UX 확장 방식: **플러그인 우선 + 가이드/예제 강화** 확정 (`mount`는 예외적/국소적 사용) → `DESIGN_PHASE2.md` §2.6
...
- [x] 환경 변수 목록 문서화 (`PORT`, `BASE_PATH`, `NODE_ENV`).
...
- [x] 프레임워크 런타임 코드를 npm 배포 단위 `state-surface`로 분리 (public API + runtime + types).
- [x] 샘플 앱/사이트 코드(`routes`, `layouts`, `shared/content` 등)를 `state-surface` publish 대상에서 제외.
- [x] `state-surface` 패키지 메타 정비:
  - `name`, `version`, `exports`, `types`, `files`, `publishConfig`.
  - `exports`는 `'state-surface'`, `'state-surface/server'`, `'state-surface/client'`를 그대로 노출.
- [x] `npm pack` 기준으로 불필요 파일 누락/필수 파일 포함 검증.

### 업데이트 경로/마이그레이션 체계

- [x] `MIGRATION.md` 작성: 버전별 breaking change, 수동 수정 절차, 검증 단계.
- [ ] (선택) `state-surface migrate` codemod 엔트리 정의 (반복되는 import/설정 변경 자동화).
- [x] 릴리스 노트 템플릿에 "업데이트 절차 (`pnpm up state-surface`)" 섹션 고정.

### CLI 설계 (생성 전용)

- [x] `create-state-surface/` 디렉토리 구조 확정.
- [x] `../lithent/createLithent` 코드 참고해 CLI 진입점 구현 방식 결정.
- [x] 옵션 정책 확정 (기본: 전체 포함, 선택 옵션 최소화).
- [x] 생성 후 안내 문구 확정 (`cd`, `pnpm install`, `pnpm dev`).
- [x] CLI 도움말/README에 "업데이트는 재생성이 아니라 `pnpm up state-surface`" 명시.

### CLI 스캐폴드 소스

- [x] `scripts/build-scaffold.mjs`가 루트 파일(`client/`, `shared/`, `routes/`, `layouts/`, `server.ts`, `.prettierrc`) + `create-state-surface/overrides/`로 `scaffold/` 자동 생성.
- [x] 불필요한 파일 제외 (`engine/`, `.git`, `node_modules`, `dist`, `IMPLEMENT*.md`, `DESIGN*.md`, `PROTOCOL.md` 등).
- [x] `create-state-surface/overrides/package.json.template` — 프로젝트명 치환 + `state-surface` 의존성 버전 범위 삽입.
- [x] 스캐폴드에 `server.ts`, `client/main.ts`, `routes/_shared/hooks.ts`, `client/plugins/prism.ts` 등 루트 파일 자동 포함.
- [x] `client/styles.css`는 engine 전용 `@source` 라인 제거 후 복사.

### CLI 구현/배포 설정

- [x] `bin/create-state-surface.js` — GitHub tarball 다운로드 → `scaffold/` 추출 → 프로젝트 디렉토리 생성 → 치환.
- [x] 프로젝트명/설명 치환 처리.
- [x] Git 초기화 (`git init`) 자동 실행.
- [x] 완료 후 안내 메시지 출력.
- [x] `create-state-surface/package.json` — `"name"`, `"bin"`, `"files"` (bin + README만 포함, 스캐폴드는 GitHub에서 fetch).
- [x] `state-surface` npm `files`에서 `scaffold/` 제외.

### Baseline 테스트

- [x] 빈 디렉토리에서 `npx create-state-surface my-app` 1회 실행으로 프로젝트 생성.
- [x] 생성 프로젝트의 `package.json`이 `state-surface` 의존성을 포함.
- [x] 생성 프로젝트에서 `pnpm install && pnpm test` 통과.
- [x] 생성 프로젝트에서 `pnpm dev` 후 주요 route 200 + transition 응답 확인.
- [x] 생성 결과물의 import 경로가 `'state-surface'`, `'state-surface/server'`, `'state-surface/client'` 사용.
- [x] 생성 프로젝트에서 `pnpm build && pnpm start` → 프로덕션 모드 동작.
- [ ] 기존 프로젝트 fixture에서 `pnpm up state-surface` 후 빌드/테스트 통과 (재스캐폴딩 없이 업데이트 검증).
  - 미완료 사유: 공개 npm 버전 부재로 실제 fixture 업그레이드 경로를 이번 phase에서 검증하지 못함.
...
- [x] `createApp()` 내부에서 격리된 `templateRegistry`, `transitionRegistry` 인스턴스 생성.
- [x] `bootstrapServer()`에 레지스트리 인스턴스 주입 파라미터 추가.
- [x] `transitionRegistry`에 `clearRegistry()` 추가 (templateRegistry와 일관성).
- [x] 클라이언트측 templateRegistry는 모듈 레벨 유지 (브라우저는 단일 인스턴스).

### Phase 1 잔여 스모크

- [x] Action: examples 하위 페이지 `data-action` end-to-end.
- [x] Action: pending visual feedback.
- [x] i18n: 모든 페이지 ko/en 전환.
- [x] i18n: MPA 네비게이션 시 쿠키 유지.
- [x] basePath: `BASE_PATH=/demo pnpm dev` 전체 동작.
- [x] Guide: `/guide/quickstart` 4단계 표시.
- [x] Guide: concept 가이드 analogy/debug 섹션.
- [x] Guide: ko/en 구조 동일.
- [x] Guide: 모바일 레이아웃.
- [x] Chat: 메시지 즉시 표시, 스트리밍 누적, abort, 언어 전환.
- [x] Chat: 템플릿에 `mount`/`state` import 없음.

### Phase 2 추가 스모크

- [x] 프로덕션 빌드 후 전체 route 접근 가능.
- [x] `npx create-state-surface` 생성물이 기준 사이트와 시각적 일치.

### Baseline 테스트 자동화

- [x] 테스트 간 레지스트리 격리 확인 (병렬 테스트에서 상태 누수 없음).
- [x] basePath 통합 테스트 추가 (Express 라우트 접근, transition URL, 쿠키 Path).
- [x] NDJSON malformed JSON 청크 스킵 테스트.
- [x] SSR production 에러 응답 테스트.
- [x] 스캐폴드는 루트 파일에서 `scripts/build-scaffold.mjs`로 자동 생성되므로 별도 패리티 테스트 불필요 (기존 `templateParity.test.ts` 제거).

---

## Phase 2-13.5: 플러그인 사용 가이드 강화 (Guide-Only)

> 설계: `DESIGN_PHASE2.md` §2.6

**Entry**: Phase 2-13 완료.
**Exit**: 플러그인 사용 기준이 template 가이드에 문서화됨.

> **결정**: 별도 `/features/plugins` 데모 페이지는 불필요 — Prism/guideToc이 이미 `/guide/*`에서 실동작 중이므로 인위적 재현의 가치가 낮다. 가이드 문서 강화만 수행.

### Guide 문서 강화

- [x] `shared/content.ts`의 template guide에 `plugins` 섹션 추가 (en/ko):
  - paragraph: 플러그인 사용 기준 (DOM 보정 효과만 담당)
  - warning: anti-pattern (`onUpdate`에서 상태 결정/네트워크 호출 금지)
  - code × 2: `prismPlugin`, `guideTocPlugin` 최소 예시
  - callout: CSS-only vs Template 수정 vs Plugin 판단 기준
- [x] en/ko 가이드 구조 parity 유지 (`shared/guideContent.test.ts` 기준).
- [x] `shared/guideContent.test.ts`에 `TEMPLATE_EXTRA_SECTION_IDS = ['plugins']` 추가.

### 스캐폴드 동기화

- [x] `scaffold/`는 `pnpm build` 시 루트 파일에서 자동 생성되므로 수동 동기화 불필요.

### Baseline 테스트

- [x] `pnpm test -- shared/guideContent.test.ts` 통과 (가이드 schema/i18n parity 유지).
- [x] `pnpm test -- engine/server/phase213Smoke.test.ts` 통과.
- [x] 스캐폴드 패리티: 루트 파일에서 자동 생성되므로 별도 테스트 불필요 (기존 `templateParity.test.ts` 제거).
- [x] `pnpm dev`에서 `/guide/template` 수동 확인:
  - Plugin Patterns 섹션 노출
  - Prism 하이라이팅/라인넘버 정상.
...
**Entry**: Phase 2-13.5 완료.
**Exit**: 신규 프로젝트 스캐폴딩 → 전체 파이프라인 동작 확인.

- [x] 빈 디렉토리에서 `create-state-surface` → `pnpm install` → `pnpm test` → 전체 통과.
- [x] `pnpm dev` 경로의 런타임 검증(동등 실행: `node --import tsx server.ts`) → SSR 렌더링 → data-action → NDJSON 스트리밍 → DOM 업데이트.
- [x] `pnpm build && pnpm start` → 프로덕션 모드 동작.
- [x] `BASE_PATH=/demo` 경로(dev 실행 기준) → 서브패스 동작.
- [x] ko/en 언어 전환 → 쿠키 유지 → MPA 네비게이션.
- [x] 훅 기반 i18n 동작 검증 (엔진에 i18n 하드코딩 없음 확인).
- [x] 플러그인 기반 Prism 동작 검증 (엔진에 Prism 하드코딩 없음 확인).

### Baseline 테스트 (2026-03-06)

- [x] `pnpm test` (root) — `35 files, 542 tests` pass.
- [x] 빈 디렉토리 스캐폴딩:
  - 기본 `pnpm install`은 미배포 패키지(`state-surface`)로 404 확인.
  - `STATE_SURFACE_VERSION=file:/Users/superlucky84/project/state-surface`로 재생성 후 `pnpm install && pnpm test` 통과 (`2 files, 147 tests`).
- [x] dev 런타임 검증 (`node --import tsx server.ts`):
  - `GET /` SSR HTML + `<h-state ... data-ssr-hash>` 확인.
  - `POST /transition/search` NDJSON(`state` → partial `state` → `done`) 확인.
  - `POST /transition/switch-lang` `Set-Cookie: lang=ko` 확인.
  - `GET /` with `Cookie: lang=ko` 시 ko 상태 SSR 확인.
- [x] DOM 업데이트 파이프라인 검증: `engine/server/demoIntegration.test.ts` 포함 전체 회귀 통과 (서버 NDJSON → 클라이언트 상태 적용).
- [x] 프로덕션 검증: 스캐폴딩 앱에서 `pnpm build && pnpm start` 후 `GET /examples/search`, `POST /transition/search` 정상.
- [x] 서브패스 검증: `BASE_PATH=/demo node --import tsx server.ts` 후
  - `GET /demo/examples/search` 200, `GET /examples/search` 404
  - `POST /demo/transition/search` NDJSON 정상
  - `POST /demo/transition/switch-lang`의 `Set-Cookie`에 `Path=/demo` 확인.
- [x] i18n/Prism 경계 검증:
  - `engine/` 내 i18n/Prism 직접 의존 import 없음 (`rg` 확인).
  - i18n 훅은 `routes/_shared/hooks.ts`에 위치.
  - Prism/guideToc 플러그인은 `client/main.ts`에서 등록.

### (선택) E2E 자동화

- [ ] E2E 도구 도입 (DC-04에서 v0.1.0 범위는 생략으로 확정, 필요 시 후속 단계에서 재개).
...
- [x] 서버 로그에 요청 메서드/URL/상태코드 기본 출력.
- [x] `NODE_ENV=production`에서 verbose 로그 억제.
- [x] (선택) 간단한 로그 레벨 시스템 (`debug`, `info`, `warn`, `error`).

### 구현 메모 (2026-03-06)

- [x] `engine/server/logger.ts` 추가:
  - 기본 레벨: `production -> info`, `test -> warn`, 그 외 `debug`.
  - 임계치 기반 출력: `debug/info/warn/error`.
- [x] `createApp()` 옵션에 `logLevel` 추가 (`StateSurfaceServerOptions`).
- [x] 요청 로깅 미들웨어 추가:
  - 형식: `[HTTP] METHOD URL STATUS N.Nms`
  - 응답 `finish` 시점에 상태코드/elapsed time 기록.
- [x] 개발 시작 로그를 `info + debug` 레벨로 분리 (routes/debug-overlay는 debug).
- [x] 전환 훅 후처리 실패 로그를 `logger.error`로 통일.
- [x] 서버 공개 API에 `createLogger`, `LogLevel`, `Logger` export 추가.

### Baseline 테스트

- [x] `NODE_ENV=production`에서 debug 로그가 stdout에 출력되지 않음 (`engine/server/logger.test.ts`).
- [x] 요청 로그 포맷 검증: `engine/server/logger.test.ts`에서 `[HTTP] GET /examples/search 200` 검증.
- [x] 전체 회귀: `pnpm test` 통과 (`36 files, 546 tests`).
...
- [x] `pnpm build && pnpm start` + `/examples/search`, `/transition/search` 스모크 통과.
...
| ~~14~~ | ~~**2-13 싱글턴→인스턴스 + Test Hardening**~~ | ~~Medium~~       | ~~§8~~      | ✅ 완료                                    |
| ~~15~~ | ~~**2-13.5 플러그인 가이드 강화**~~       | ~~Medium~~       | §2.6        | ✅ 완료 (Guide-Only, 데모 페이지 제외)     |
| ~~16~~ | ~~**2-14 Integration Test**~~             | ~~Medium~~       | ~~—~~       | ✅ 완료                                    |
| ~~17~~ | ~~2-15 로깅~~                             | ~~Low~~          | ~~—~~       | ✅ 완료                                    |
...
2-14 Integration Test — 2-13.5 완료 후 수행
2-15 로깅 — 선택, 독립
```
