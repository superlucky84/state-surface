...
## Testing Guidelines
Tests are written with Vitest and are colocated with their code under `client/`, `server/`, `shared/`, and `routes/`. Run all tests with `pnpm test`, or filter locally with `pnpm test -- -t "keyword"`. There is no explicit coverage threshold configured; add or update tests for any new behavior or protocol changes, especially in `shared/` contracts.
