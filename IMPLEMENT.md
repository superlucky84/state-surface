...
- [x] Migrate existing demo to route files:
  ...
  - [x] `routes/examples/search.ts` → `/examples/search`
...
| `GET /examples/search`      | `search:input`, `search:results`    | `page:header`, `system:error`  |
...
- [x] Verify `routes/examples/search.ts` already follows the principle (search-only slots).
...
- [ ] Smoke check: examples (search/streaming/actions/chat) 페이지 액션이 `data-action`으로 end-to-end 동작.
...
- 기존 article/search 데모를 교체 (chat은 Phase 13에서 별도 구현, 모두 examples/ 하위로 이동)
...
| `/examples/streaming` | 스트리밍 데모 — 프레임 흐름 실시간 시각화                  | Full/partial, `removed`, error frame               |
| `/examples/actions`   | 액션 플레이그라운드 — 버튼, 폼, scoped pending 체험        | `data-action`, form submit, `data-pending-targets` |
| `/examples/search`    | StateSurface 기능/개념 검색                                | Form `data-action`, pending 상태                   |
...
- [x] Create streaming demo page (`/examples/streaming`):
  - [x] `routes/examples/streaming.ts` — 스트리밍 시각화 surface.
  - [x] `routes/examples/streaming/transitions/streamDemo.ts`:
    - [x] Full frame → partial (changed) → partial (removed) → error → done 시퀀스.
    - [x] 각 프레임 타입을 의도적으로 시연.
  - [x] `routes/examples/streaming/templates/demoControls.tsx` — 프레임 발사 버튼들.
  - [x] `routes/examples/streaming/templates/demoTimeline.tsx` — 프레임 도착 타임라인.
  - [x] `routes/examples/streaming/templates/demoOutput.tsx` — 현재 activeStates 시각화.
  - [x] `removed` 키 시연 (이전에 빠져있던 기능).
  - [x] Error frame 시연.
- [x] Create actions playground page (`/examples/actions`):
  - [x] `routes/examples/actions.ts` — 액션 플레이그라운드 surface.
  - [x] `routes/examples/actions/transitions/actionDemo.ts` — 다양한 액션 처리.
  - [x] `routes/examples/actions/templates/actionsPlayground.tsx`:
    - [x] 버튼 `data-action` 예제.
    - [x] Form `data-action` 제출 예제.
    - [x] `data-pending-targets` scoped pending 예제.
    - [x] 다중 action 버튼 예제.
  - [x] `routes/examples/actions/templates/actionsLog.tsx` — 액션 이벤트 로그 표시.
- [x] Redesign search page (`/examples/search`):
  - [x] `routes/examples/search.ts` — StateSurface 기능/개념 검색으로 콘텐츠 변경.
  - [x] `routes/examples/search/transitions/search.ts` — 기능 목록에서 검색.
...
- [x] 언어 콘텐츠 데이터 구조 정의:
  ...
  - [x] 스트리밍(`/examples/streaming`): controls, timeline 라벨 한글화.
  - [x] 액션(`/examples/actions`): playground, log 라벨 한글화.
  - [x] 검색(`/examples/search`): 검색 항목 + UI 라벨 한글화.
- [x] 쿠키 인프라:
...
| ----------- | -------------------------------------------- | ----------------------------- |
| `GET /examples/chat` | `chat:messages`, `chat:input`, `chat:typing` | `page:header`, `system:error` |
...
- [x] Create `routes/examples/chat.ts` with chat-specific surface (`chat:messages`, `chat:input`, `chat:typing`).
- [x] Create `routes/examples/chat/transitions/chat.ts`:
...
- [x] Create chat templates:
  - [x] `routes/examples/chat/templates/chatMessages.tsx` — message list with `cacheUpdate` per message.
  - [x] `routes/examples/chat/templates/chatInput.tsx` — input form.
  - [x] `routes/examples/chat/templates/chatTyping.tsx` — typing indicator.
...
- [x] Update `pageHeader.tsx` nav with `/examples/chat` link.
- [x] Add tests:
...
첫 방문자 학습 순서:
  ...
  /examples/*         ← 실습: 각 개념을 브라우저에서 직접 조작
```
...
섹션 8: 다음 단계
  ...
  - 실습 페이지 링크 (/examples/streaming, /examples/actions)
```
...
**배경:**
현재 챗봇(`/examples/chat`) 데모는 `ChatMessages`와 `ChatCurrent` 템플릿이 Lithent `mount`/`state`/`updateCallback`으로
...
- [x] `routes/examples/chat/transitions/chat.ts`
...
- [x] `routes/examples/chat/templates/chatMessages.tsx`
...
- [x] `routes/examples/chat/chat.test.ts` 업데이트:
...
- [ ] Smoke check:
  - [ ] `/examples/chat`에서 메시지 전송 → 유저 메시지 즉시 표시.
...
- 가이드/예제 페이지(`/`, `/guide/*`, `/examples/*`)가 모두 포함.
...
- 2026-02-19: Phase 14 runtime smoke fixed — bootstrap root resolution corrected (`engine/server/bootstrap.ts`), transition compatibility export fixed (`server/transition.ts`), and `pnpm dev` verified for all routes + examples (chat/search) NDJSON transitions.
