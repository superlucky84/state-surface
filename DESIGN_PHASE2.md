...
### 2.6 플러그인 사용 가이드 강화 (Guide-Only)

플러그인 시스템은 엔진의 공식 확장 포인트다.
문서가 약하면 사용자가 `mount`와 플러그인 사이에서 과도한 설계 결정을 해야 하므로, 가이드에 플러그인 사용 기준을 명시한다.

> **결정**: 별도 `/features/plugins` 데모 페이지는 만들지 않는다. Prism 하이라이팅과 guideToc 스크롤은 이미 `/guide/*` 페이지에서 실제로 동작하고 있어 인위적 재현의 가치가 낮다. 가이드 문서 강화만으로 충분하다.
...
1. Guide 확장
   - `guide/template`에 “Plugin Patterns” 섹션 추가:
...
| 10 | §2.6 플러그인 가이드 강화 (Guide-Only) | 2-13.5 (신규) | **Medium** — 도입/온보딩 품질 |
